...
include IntroductionGuide, HelpTab, LoginPage, NavigationBar
...
feature 'Introduction guide' do
  it 'has got all crucial elements' do
    ...
    (1..7).each do |page_number|
      ...
      if page_number == 1
        expect(page).not_to have_css(INTRO_PREV_BUTTON)
      else
        expect(page).to have_css(INTRO_PREV_BUTTON)
      end

      if page_number == 7
        expect(page).not_to have_css(INTRO_NEXT_BUTTON)
      else
        find(INTRO_NEXT_BUTTON).click
      end
    end
